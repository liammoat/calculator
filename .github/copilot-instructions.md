...
2.  **Use shared utilities and components**:
    - Import calculation logic from [src/utils](src/utils) (conversion, geometry, sheetMetal, formatting)
    - Use shared UI components from [src/components/shared](src/components/shared) (UnitSelector, ResultCard, KFactorInput, ValidationMessage)
    - Keep calculator-specific visualization components in [src/components/shared](src/components/shared) if reusable
3.  **Register** in [src/data/calculators.ts](src/data/calculators.ts#L1):
    ```ts
    ...
    ```
4.  **Verify** rendering via CalculatorPage resolution. No route changes needed.

## Calculator Architecture Patterns

- **Separation of Concerns**: Decouple calculation logic from UI components
  - Pure calculation functions in `src/utils/` (no React, no state)
  - UI components in `src/calculators/` use these functions
  - Shared UI components in `src/components/shared/`
- **Shared Utilities**:
  - `src/utils/conversion.ts` - Unit conversions (length, area, etc.)
  - `src/utils/geometry.ts` - Geometric calculations (area, volume, etc.)
  - `src/utils/sheetMetal.ts` - Sheet metal calculations (bend allowance, deduction, etc.)
  - `src/utils/formatting.ts` - Number formatting and parsing
- **Shared Components**:
  - `UnitSelector` - Consistent unit selection dropdowns
  - `ResultCard` - Styled result display with optional details
  - `KFactorInput` - K-factor preset selector with manual override
  - `ValidationMessage` - Error message display
  - Visualization components for specific calculator types (CircleVisualization, BendVisualization)
- **State Management**: Use `useState` for inputs and results, `useMemo` for computed values
- **Validation**: Validate inputs before calculation; show helpful error messages
