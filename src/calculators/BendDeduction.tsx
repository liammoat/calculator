...
const BendDeduction: React.FC = () => {
  ...
  const calculate = () => {
    ...
    const bd = calculateBendDeduction(params);
  ...
  const resultDetails = useMemo(
    ...
    [angleDeg, insideRadius, thickness, unitSystem, kFactor, sbResult, baResult]
  );
    ...
    <Card>
      <CardContent>
        ...
        <Stack spacing={2} sx={{ mt: 2 }}>
          ...
          {bdResult && isValid && (
            <Stack spacing={2}>
              <ResultCard value={bdResult} unit={unitSystem} details={resultDetails} />
              <BendVisualization
                angleDeg={angleVal}
