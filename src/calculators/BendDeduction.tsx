...
const BendDeduction: React.FC = () => {
  ...
  const [bdResult, setBdResult] = useState<string>('');
  const [sbResult, setSbResult] = useState<string>('');
  const [baResult, setBaResult] = useState<string>('');
  ...
  const calculate = () => {
    ...
    const sb = (radiusVal + thicknessVal) * Math.tan((angleVal * Math.PI) / 360);
    const ba = thetaRad * (radiusVal + kVal * thicknessVal);
    const bd = 2 * sb - ba;
    setSbResult(roundByUnit(sb, unitSystem));
    setBaResult(roundByUnit(ba, unitSystem));
    setBdResult(roundByUnit(bd, unitSystem));
  };
  ...
  }, [bdResult, isValid, radiusVal, thicknessVal, kVal, angleVal]);

  const ariaLabel = useMemo(() => {
    if (!bdResult || !isValid) return 'Bend deduction visualization: no result';
    return `Bend deduction visualization: ${angleVal}° bend, inside radius ${insideRadius} ${unitSystem}, thickness ${thickness} ${unitSystem}, K-factor ${kFactor}, setback ${sbResult} ${unitSystem}, bend allowance ${baResult} ${unitSystem}, bend deduction ${bdResult} ${unitSystem}`;
  }, [
    bdResult,
    isValid,
    angleVal,
            ...
            <Stack spacing={2}>
              <Card sx={{ bgcolor: 'primary.main', color: 'primary.contrastText' }}>
                <CardContent>
                  ...
                  <Typography variant="h4">{bdResult}</Typography>
                  ...
                  <Typography variant="body2" sx={{ mt: 0.5 }}>
                    SB: {sbResult} {unitSystem} | BA: {baResult} {unitSystem}
                  </Typography>
                </CardContent>
              </Card>
